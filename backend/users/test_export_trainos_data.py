...
class PortableExportTests(TestCase):
    ...
    def create_full_export_data(self):
        ...
        batch = ImportBatch.objects.create(user=self.user, original_filename="source.tcx", uploaded_file=r"C:\Users\private\source.tcx", errors=[{"line": 4, "message": "ignored"}])
        ...
        ActiveWorkout.objects.create(user=self.user, template=template, logged_sets=[{"set": 1, "done": True}])
